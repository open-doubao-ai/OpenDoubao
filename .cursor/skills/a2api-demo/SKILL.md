...
- Sensitive writes (default `delete`) go to Admin approval; other writes auto-execute and store `auto_approved` audit rows (`opendoubao/data/approvals.jsonl`).
...
- UI copy is i18n via `i18next` in opendoubao (`opendoubao/client/i18n/`) and admin (`admin/client/i18n/`), locales `en` / `zh-CN`; Settings → **UI Language** (reload on change). Both apps share `localStorage` key `a2api.uiLocale`. Separate from **AI Language** (LLM reply language).
- Chinese NLP matching may remain for intent; opendoubao chip `data-msg` follows UI locale.
...
- `npm run typecheck` in `@a2api/opendoubao` when touching TS
